...
-- COMO QUER APENAS 1 MÉDICO (O QUE TEVE MAIS CONSULTAS) UTILIZAMOS O TOP 1
 ...
 -- PRIMEIRA RESOLUÇÃO FORNECIDA PELO PROFESSOR.
 ...
 */
 ...
 -- MINHA RESOLUÇÃO 
 -- O PROFESSOR DISSE QUE NÃO É O IDEL MAS É ACEITÁVEL
 SELECT
	 PAC.NOME
	,CON.DTCONSULTA AS 'DATA CONSULTA'
	,CON.CODPACIENTE
 ...
 LEFT JOIN 
	(
		...
		WHERE LEFT(C.DTCONSULTA, 7) = '2021-04'
	) AS CON
	ON PAC.CODIGO = CON.CODPACIENTE
 ...
 FROM PRONTUARIO.CLINICA.PACIENTE AS PAC
 LEFT JOIN PRONTUARIO.CLINICA.CONSULTA AS CON
	ON PAC.CODIGO = CON.CODPACIENTE AND LEFT(CON.DTCONSULTA, 7) = '2021-04'
